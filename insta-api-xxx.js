...
module.exports = class Instagram {
  ...
  async getStories(userid) {
    ...
    return new Promise((resolve, reject) => {
      ...
      fetch(
        `https://www.instagram.com/graphql/query/?query_hash=45246d3fe16ccc6577e0bd297a5db1ab&variables={"reel_ids":["${userid}"],"tag_names":[],"location_ids":[],"highlight_reel_ids":[],"precomposed_overlay":false}`,
        {
        ...
        .then(async (res) => {
          ...
          for (let story of a.data['reels_media'][0].items) {
            story.url = story.video_resources
      ...
      ;(async function() {
        while (toReturn.posts.length < num && !stop) {
          await new Promise((next) => {
            let afterFetch = `https://www.instagram.com/graphql/query/?query_hash=bd0d6d184eefd4d0ce7036c11ae58ed9&variables={"id":${userid},"first":${num -
              toReturn.posts.length}${first !== '' ? `,"after":"${first}"` : ''}}`
            fetch(afterFetch, {
              headers: headers,
            })
              .then(async (res) => {
    ...
    return new Promise((resolve, reject) => {
      got('https://www.instagram.com/p/' + shortcode)
        .then((r) => {
          ...
          if (
            !jsonData['entry_data']['PostPage'][0]['graphql']['shortcode_media']['edge_sidecar_to_children'] ||
            !jsonData['entry_data']['PostPage'][0]['graphql']['shortcode_media']['edge_sidecar_to_children']['edges']
          ) {
            if (ogvideo && ogvideo['0']) {
              //If video
    ...
    return new Promise((resolve, reject) => {
      ...
      ;(async function() {
        while (toReturn.comments.length < num && !stop) {
          await new Promise((next) => {
            let afterFetch = `https://www.instagram.com/graphql/query/?query_hash=f0986789a5c5d17c2400faebf16efd0d&variables={"shortcode":"${shortcode}","first":${num -
              toReturn.comments.length}${first !== '' ? `,"after":"${first}"` : ''}}`
            fetch(afterFetch, {
              headers: headers,
            })
              .then(async (res) => {
        ...
        toReturn.comments.sort(function(a, b) {
